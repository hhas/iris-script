...
indirect enum Pattern: CustomDebugStringConvertible, ExpressibleByArrayLiteral {
    ...
    case test((Token.Form)->MatchResult)
    ...
    func match(_ form: Parser.Reduction) -> MatchResult { // needs to take Token, not Form, in order to match adjoining whitespace (this means parser stack has to capture Token)
        ...
        case .name:
            switch form {
            case .quotedName(_), .unquotedName(_): return .fullMatch // TO DO: what about undifferentiated .letters, .symbols, etc? can we guarantee those have been reduced to name tokens by the time they reach here? (note: can't do anything with them if they do reach here)
            default: return .noMatch
            }
        case .label:
            switch form {
            case .quotedName(_), .unquotedName(_), .operatorName(_): return .fullMatch // TO DO: ditto
            default: return .noMatch
            }
