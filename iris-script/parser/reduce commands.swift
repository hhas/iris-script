...
extension Parser {
    ...
    func addMatchersForNestedCommand(from startIndex: Int, to stopIndex: Int) {
        ...
        self.stack[startIndex].matches += matchers
        if startIndex + 1 < stopIndex { // if there's more tokens after the name
            ...
            let form = self.stack[startIndex+1].form
            self.stack[startIndex+1].matches += matchers.filter{ $0.provisionallyMatches(form: form) }
        }
    }
