...
public struct NativizedCoercion<ElementType: SwiftCoercion>: CallableNativeCoercion {
    ...
    public func call<T>(with command: Command, in scope: Scope, as coercion: T) throws -> T.SwiftType where T : SwiftCoercion {
        if command.arguments.isEmpty { // TO DO: is this shortcut safe? (it saves calling nativeCoercion.call)
            return try coercion.coerce(self, in: scope)
        } else {
            guard let nativeCoercion = self.elementType.nativeCoercion as? CallableNativeCoercion,
                !(nativeCoercion is NativizedCoercion<ElementType>) else { // extra check is needed here to avoid infinite recursion if the returned coercion is just the original SwiftCoercion in a nativized wrapper; i.e. only true NativeCoercions can/should implement CallableNativeCoercion protocol and a call() method
