...
// TO DO: what would be needed to support 'high-school' algebraic syntax? (e.g. `3𝓍²+5𝓎－1`; e.g. superscript nos. appear as .letters so would need a lexer to identify those; we'd also need a lexer that decomposes .letters to single-char tokens; pow operator `^` followed by digit would need rewritten as superscript no. [in absence of a keyboard sequence for entering superscript directly])
