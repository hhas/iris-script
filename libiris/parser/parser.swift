...
//            }
       ...
       // automatically reduce fully matched atomic operators and list/record/group/block literals (i.e. anything that starts and ends with a static token, not an expr, so is not subject to precedence or association rules)
        ...
        let completedIDs = currentMatches.filter{ $0.isLongestFullMatch }.map{ $0.originID } // not entirely sure why `do…done` appears in currentMatches as both completed and partial match (the `done` keyword should _always_ terminate the current `do` block, so the incomplete match ought to be pruned by the conjunction matching logic above); for now, work around it here // TO DO: there might also be some bugginess when reducing backwards from right-delimiter: that should pick up the `do…done` block even if it wasn’t preemptively reduced here, but fails to match the (do/done?) keyword token instead
        let partialIDs = currentMatches.filter{ !$0.isLongestFullMatch && !completedIDs.contains($0.originID) }.map{ $0.groupID }
