...
public struct PrimitiveHandler: Handler {
    ...
    public init(interface: HandlerType, action: @escaping Call, in handlerScope: Scope) {
    ...
    public func call<T: SwiftCoercion>(with command: Command, in commandScope: Scope, as coercion: T) throws -> T.SwiftType {
        ...
        do {
            // TO DO: KLUDGE: PrimitiveHandler.Call currently returns Value, which we coerce to final return type here
            let result = try self.action(command, commandScope, self, self.lexicalScope, coercion.nativeCoercion) // TO DO: function wrapper currently ignores `coercion` (it's passed here on assumption that glue code will eventually intersect it with its interface.returnType, although how that native/Swift value will be returned from Call wrapper function remains to be seen)
            return try coercion.coerce(result, in: commandScope)
        } catch {
